...
In all simulations, the von Neumann boundary condition \textit{(flux through a surface $\rightarrow\sigma\nabla\phi\cdot\vec{n} = 0$)} is applied on the whole model domain. On the active and non-active electrodes, the Dirichlet boundary condition \textit{(potential on a surface $\rightarrow\phi = \phi_0$, $\phi = -\phi_0$)} is applied, respectively. The desired injected current determines the magnitude of $\phi_0$.
