...
Due to the layered structure (\autoref{fig:cortical_layers}) of the neocortex, \gls{TMS} induced electric fields affect mostly interneurons and collaterals of the tangentially aligned pyramidal cells, whereas the overall electric field induced by electrical stimulation is normal to the cortical surface thus affecting mostly \textit{layer V} pyramidal neurons as most of them are perpendicular to the cortex. The end result is that \gls{TMS} induced fields have short penetration depth, while \gls{tES} induced fields can reach deep in the brain at the cost of spatial resolution. As such, \gls{tES} is mostly used to try and reach deep targets.
% Maybe used another phrase than "deep in the brain" because you have already used it three times :P
\gls{DBS} and especially the non-invasive methods to achieve it is an emerging need, since most of the functionality of key brain components lies deep in the brain. Although there are successful invasive methods that can alleviate Parkinson's symptoms, non-invasive methods can offer greater scalability and lower risk for the patient. The challenge posed by the non-invasive methods is the lack of spatial resolution and precise targeting of the desired area, which is usually in the order of tens of millimeters (e.g. pituitary gland \cite{Yadav2017_pituitary}).

On 2017, a new method, which seemingly revolutionizes the way non-invasive \gls{DBS} is accomplished by accommodating clever tricks or \textit{"hacking"} the physics of neurons, is brought to light by Nir Grossman and his team \cite{Grossman2017} for the first time. The \paper{Grossman}{Grossman2017} method uses a pair of electrodes operated on a high frequency alternating current, whose envelope oscillates at a desired frequency through setting different frequencies between the electrode pairs. The admirable resulting advantage is the ability to reach deep targets and stimulate only the areas of interest, without stimulating the surrounding areas or the whole brain.

The Grossman method shows promise in the future of \gls{DBS} and it is studied in this work to assess the potential use in humans, since in the original work \cite{Grossman2017} only spherical and simple phantom models were featured, in conjunction with successful test runs in murine subjects. Expanding the study on realistic human brain models will aid in the understanding of the \gls{tTIS} specifics and testing on healthy individuals will consolidate the method's effectiveness. The utmost goal of this work is to create optimized and personalized treatment planning for humans based on rigorous computer simulations. 

As a closing remark, it has been a firm belief of the author that the greatest benefit to science and society comes from open knowledge sharing and to abide to this notion, all code, designs, texts and any material originally produced for this thesis are publicly available on the GitLab repository \cite{thesis_repo}.
