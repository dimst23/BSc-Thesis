...
Although \gls{tDCS} is good for an overall cortical stimulation, the lack of focality is one of the key issues. As it can be seen in \draft{fig:tcds\_pattern} the electric current flows though a particularly large area, potentially affecting unwanted areas.

\draft{figure for tDCS pattern goes here}
...
In many aspects \gls{tACS} and \gls{tDCS} are the same. As mentioned before, the key component where \gls{tACS} \draft{arises} is the neural firing synchronization. Same as \gls{tDCS}, this method also lacks focality as shown in \draft{fig:tacs\_pattern} and there is one more issue in the game now.

\draft{figure with the tACS pattern goes here}
...
\draft{refer why stimulating with low frequency currents in not the best}
...
\draft{
    \begin{itemize}
        \item Refer to the physics behind the method
        \item Provide interference diagrams to back it up
        \item Provide some initial diagrams showing the field distribution in the head, both mine and from Grossman
        \item Refer to the potential utilization, targets, methods, improvement
    \end{itemize}
}
