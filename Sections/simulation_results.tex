...
In this work, the attention will be shifted towards the electrode position variation and its subsequent effects. One can imagine that moving the electrodes or changing the combination used can impact the final pattern. It is evident from \autoref{fig:elec_position_variation} that using the combination \draft{just one step above} the previous one can have a significant impact on the field distribution.
