...
Model anatomy is one of the crucial factors that must be considered when simulating with \gls{tTIS} but it is not the only player, since the current ratio between the two electrodes and the position thereof can pose a major influence on the results.

As shown by \paper{Rampersad}{Rampersad2019}, the current ratio has significant impact regarding the maximum potential achieved in a given volume. This dependence can be evidently seen in \autoref{fig:mod_env_amplitude_var}, due to the fact that when the two electrodes are not placed symmetrically, the pair closest to the region of interest takes over. The resulting modulation amplitude is much lower than the one seen in \autoref{fig:modulation_showcase}.
