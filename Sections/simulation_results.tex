...
The entirety of this work is based on the method introduced by \paper{Grossman}{Grossman2017}, which has been tested on murine brains and the results were closely aligned the simulated ones. Although the method can produce the desired results when applied on the murine brain, it is much more challenging to achieve and accurately simulate the required outcome on the human brain.

\paper{Grossman}{Grossman2017} simulation models were based on a simple spherical layered model with uniform density of each material. Such an approach is accurate for small brains, like murine brains, but it fails for the human case, where the structure is very complex and greatly varying between subjects. One way to better approach the problem on humans is to use detailed models that capture the non-uniformities and fine geometric details, giving results closer to reality.
...
% What is a radius fraction?
The simple spherical model is comprised of 4 layers in total, with varying thickness each. The layer thicknesses are included in \autoref{tab:spherical_layers} as radius fractions and stem from Grossman et al. \cite[Figure S2, J and K]{Grossman2017} \textit{(referred hereafter as paper)}. Additionally, the conductivity values used in the paper are summarized in \autoref{tab:grossman_conductivity_vals}.
% I don't think it is a really good idea to use "the paper" because someone might not be reading your text sequentially and have no idea where you refer to. You can just include the number, e.g. [6], without the Grosmman et al blah blah, but I don't like "the paper" at all :P
\begin{table}[!ht]
\centering
...
To benchmark the software developed for this work, the same configurations as in the paper were used and the results can be seen in \autoref{fig:grossman_thesis_comparison}.
